...
- **The repo `.npmrc` contains a plaintext token.** Pre-existing, REVIEW_REQUIRED. Do not modify it
  without asking.
- **Not a git repository yet** (`git init` has never been run), so `.gitignore` is unverified.
...
| `src/` | `pages`, `components`, `fixtures`, `services`, `models`, `utils` | Playwright Test Generator |
