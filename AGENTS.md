...
5. **No agent approves its own output.**
   ...
   files.
   A step definition that deliberately throws to keep a `MANUAL_ONLY` scenario traceable is
   classified `MANUAL_ONLY_PLACEHOLDER`; a failure whose error text cites an already-recorded
   `BLOCKER-*`/`AMB-*` id is classified `KNOWN_AMBIGUITY`. Neither is healed, filed, or allocated a
   `DEF-ID` — both route straight to `RTM_UPDATE` (see `src/utils/failure-triage.ts`).
...
- **A scenario needing a second identity** (a role distinct from the master `ECORE_USERNAME`
  account, e.g. "Approver") uses `env.requireEcoreLoginAs('<ROLE>')`, never a second hard-coded
  credential. The role must first be declared, non-secretly, in `config/test-users.json` (see
  `config/test-users.json.example`) — that file names only an `envPrefix` pointer and is safe to
  commit; the actual `<envPrefix>_USERNAME`/`_PASSWORD` pair lives only in `.env`. A role's
  existence is a business fact and is never invented by an agent — see "Environment configuration"
  in [README.md](README.md) for the full setup steps.
