...
- **If this exploration leaves a real, live side effect standing in the shared environment**
  (a fixture created and intentionally left in place, a state transition that has no reversible
  teardown - see "Environment gotchas" in `AGENTS.md` for the confirmed Work Queue example), do not
  report the investigation complete until every other approved scenario that shares the same
  resource pool (the same collection, queue, or record) has been identified and re-verified against
  it. A side effect left standing without this check risks silently breaking scenarios that never
  touch the thing you changed - the discovery-cache mechanics in
  `.github/instructions/playwright-automation.instructions.md` ("Multi-candidate discovery
  helpers") are exactly how that happens.
